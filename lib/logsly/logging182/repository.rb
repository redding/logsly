...
module Logsly::Logging182
  ...
  class Repository
    ...
    def initialize
      @masters = []
      @h = {:root => ::Logsly::Logging182::RootLogger.new}

      # configures the internal logger which is disabled by default
      logger = ::Logsly::Logging182::Logger.allocate
      logger._setup(
          to_key(::Logsly::Logging182),
          :parent   => @h[:root],
          :additive => false,
          :level    => ::Logsly::Logging182::LEVELS.length   # turns this logger off
      )
      ...
      if defined?(@singleton__instance__)
        ...
        class << self
          ...
          if defined?(nonce::FirstInstanceCall)
            ...
            Singleton.__init__(::Logsly::Logging182::Repository)
          end
        end
      end
      return nil
